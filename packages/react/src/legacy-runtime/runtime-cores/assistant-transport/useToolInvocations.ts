...
import { AssistantMetaTransformStream, type ReadonlyJSONValue } from "assistant-stream/utils";
...
type UseToolInvocationsParams = {
	...
	setToolStatuses: (
		updater:
			...
			| ((prev: Record<string, ToolExecutionStatus>) => Record<string, ToolExecutionStatus>),
	) => void;
};
...
export function useToolInvocations({ state, getTools, onResult, setToolStatuses }: UseToolInvocationsParams) {
	...
	useEffect(() => {
		const processMessages = (messages: readonly (typeof state.messages)[number][]) => {
